...
interface Vehicle {
  ...
  name: string;
  numero: string;
  type: keyof typeof VEHICLE_TYPES; // Assure que 'type' est une clé valide de VEHICLE_TYPES
  lastUpdate: Date;
...
function App() {
  ...
  const DashboardView = () => {
    ...
    const alerts = vehicles.filter(v => {
      // Correction: utiliser .getTime() pour une opération arithmétique valide
  ...
  const VehicleDetailView = () => {
    if (!selectedVehicle) {
        // Retourne à la liste si selectedVehicle n'est pas défini (sécurité TypeScript)
        return (
            <div className="space-y-6">
                <button onClick={() => setView('vehicles')} className="text-blue-600 hover:underline">← Retour</button>
                <p className="text-red-500">Erreur: Aucun véhicule sélectionné.</p>
            </div>
        );
    }
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="space-y-2">
            {authorizedOps.map(op => {
              // Cast nécessaire car 'expirations' est un Record non strictement compatible avec l'opérateur de type du filtre
    ...
    if (!selectedType) {
        return (
            <div className="space-y-6">
                ...
                <p className="text-red-500">Erreur: Aucun type d'engin sélectionné.</p>
            </div>
        );
    }
    ...
    if (!selectedOperator) {
        return (
            <div className="space-y-6">
                ...
                <p className="text-red-500">Erreur: Aucun intervenant sélectionné.</p>
            </div>
        );
    }
  ...
  return (
    // Ajouter les classes pour Tailwind CSS (supposé être chargé)
